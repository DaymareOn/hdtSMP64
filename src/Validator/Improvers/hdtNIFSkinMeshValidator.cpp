...
namespace hdt
{
	namespace
	{
		...
		std::optional<VertexLayout> parseVertexLayout(uint64_t desc)
		{
			...
			if ((out.attributes & nif::kVertexAttrTangent) != 0 &&
			    (out.attributes & nif::kVertexAttrNormal)  == 0)                      return std::nullopt;
			if (((desc >> 4) & 0x0Fu) != 0)                                           return std::nullopt;
			...
			if ((out.attributes & nif::kVertexAttrUV) != 0) {
				if (!expectOff(8,  cur)) return std::nullopt; cur += 1;
			} else if (((desc >> 8) & 0x0Fu) != 0) return std::nullopt;
			if ((out.attributes & nif::kVertexAttrNormal) != 0) {
				if (!expectOff(16, cur)) return std::nullopt; cur += 1;
			} else if (((desc >> 16) & 0x0Fu) != 0) return std::nullopt;
			if ((out.attributes & nif::kVertexAttrTangent) != 0) {
				if (!expectOff(20, cur)) return std::nullopt; cur += 1;
			} else if (((desc >> 20) & 0x0Fu) != 0) return std::nullopt;
			if ((out.attributes & nif::kVertexAttrColor) != 0) {
				if (!expectOff(24, cur)) return std::nullopt; cur += 1;
			} else if (((desc >> 24) & 0x0Fu) != 0) return std::nullopt;
			if (!expectOff(28, cur))              return std::nullopt;
			cur += 3;
			if (out.vertexSize != cur * 4u)       return std::nullopt;
			return out;
		}
			...
			uint32_t                    bsVersion)
		{
			...
			try {
...

				{
					...
					if (rawNumTri > std::numeric_limits<uint16_t>::max()) return std::nullopt;
				...
				if (out.triangles != particleTris) return std::nullopt;

				if (shapeType == nif::kTypeBSDynamicTriShape) {
					uint32_t dynSize = r.readU32();
					if (dynSize != static_cast<uint32_t>(out.numVertices) * 16u) return std::nullopt;
					r.skip(dynSize);
				}
		...
		std::optional<PartitionView> parsePartitionTolerant(
			...
			uint32_t                    bsVersion)
		{
			if (bsVersion != nif::kSSEBsVersion) return std::nullopt;
			...
			try {
				...
				if (numStrips != 0 || weightsPerVertex != 4) return std::nullopt;
				...
				if (!readBoolStrict(r, globalVB) || globalVB) return std::nullopt;
			...
			uint32_t                     stride)
		{
			...
			if (tsData.size()   != vertexMap.size() * stride) return false;
			if (partData.size() != vertexMap.size() * stride) return false;
		...
		std::vector<SkinMeshCandidate> discoverSkinMeshCandidates(const ParsedNif& parsed)
		{
			...
			for (int32_t i = 0; i < numBlocks; ++i) {
				...
				if (c.partitionBlockIndex < 0) continue;
	...
	std::vector<NifSkinMeshIssue> detectNIFSkinMeshIssues(const ParsedNif& parsed)
	{
		...
		for (const auto& c : candidates) {
			...
			if (!tsOpt) { addIssue("unsupported-trishape-layout"); continue; }

			// Step 5 (tolerant — accepts triangle copy mismatch)
			auto partOpt = parsePartitionTolerant(partBlock, parsed.bsVersion);
			if (!partOpt) { addIssue("unsupported-skin-partition-layout"); continue; }
