...
namespace hdt
{
	...
	GlobalConfig parseConfigJson(std::string_view bytes, GlobalConfig base)
	{
		...
		// --- top level --- (locale is a UI setting, so it sits at the root rather than in a section)
	...
	std::string serializeConfigJson(const GlobalConfig& c)
	{
		...
		w.Key("smp");
		w.StartObject();
		w.Key("logLevel"); w.Int(c.logLevel);
		w.Key("enableNPCFaceParts"); w.Bool(c.enableNPCFaceParts);
		w.Key("disableSMPHairWhenWigEquipped"); w.Bool(c.disableSMPHairWhenWigEquipped);
		w.Key("clampRotations"); w.Bool(c.clampRotations);
		w.Key("rotationSpeedLimit"); w.Double(c.rotationSpeedLimit);
		w.Key("unclampedResets"); w.Bool(c.unclampedResets);
		w.Key("unclampedResetAngle"); w.Double(c.unclampedResetAngle);
		w.Key("useRealTime"); w.Bool(c.useRealTime);
		w.Key("minCullingDistance"); w.Double(c.minCullingDistance);
		w.Key("autoAdjustMaxSkeletons"); w.Bool(c.autoAdjustMaxSkeletons);
		w.Key("maximumActiveSkeletons"); w.Int(c.maximumActiveSkeletons);
		w.Key("budgetMs"); w.Double(c.budgetMs);
		w.Key("sampleSize"); w.Int(c.sampleSize);
		w.Key("disable1stPersonViewPhysics"); w.Bool(c.disable1stPersonViewPhysics);
		w.Key("skipDeadActors"); w.Bool(c.skipDeadActors);
		w.Key("minScreenSizePercent"); w.Double(c.minScreenSizePercent);
		...
		w.Key("solver");
		w.StartObject();
		w.Key("numIterations"); w.Int(c.numIterations);
		w.Key("erp"); w.Double(c.erp);
		w.Key("min-fps"); w.Int(c.minFps);
		w.Key("maxSubSteps"); w.Int(c.maxSubSteps);
		w.EndObject();

		w.Key("wind");
		w.StartObject();
		w.Key("enabled"); w.Bool(c.windEnabled);
		w.Key("windStrength"); w.Double(c.windStrength);
		w.Key("distanceForNoWind"); w.Double(c.distanceForNoWind);
		w.Key("distanceForMaxWind"); w.Double(c.distanceForMaxWind);
		w.EndObject();

		w.Key("validation");
		w.StartObject();
		w.Key("mods-dir"); w.String(c.modsDir.c_str(), static_cast<rapidjson::SizeType>(c.modsDir.size()));
		w.EndObject();
