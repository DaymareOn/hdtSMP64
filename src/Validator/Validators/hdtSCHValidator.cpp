	...
	SCHValidationResult ValidatePhysicsXMLWithSchematron(const std::string& xmlPath)
	{
		...
		for (const auto& rule : schema.rules) {
			...
			for (const auto& xnode : matches) {
				...
				// If this node came from a <pattern> expansion, point the author at the pattern use
				// instead of the generated line.
				if (const PatternRange* pr = src.sourceMap.find(static_cast<std::size_t>(xnode.node().offset_debug()))) {
					v.line = pr->useLine;
					v.message += " [generated by pattern '" + pr->patternName + "' used at line " + std::to_string(pr->useLine) + "]";
				}
