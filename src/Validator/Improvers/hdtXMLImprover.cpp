...
namespace hdt
{
	namespace
	{
		...
		// Pin family instantiations nested under `node` (e.g. inside a
		// <constraint-group>) that have no template= to the current per-family
		// implicit template, so they keep their intended template once the implicit
		// (unnamed) defaults are given explicit names.
		bool pinNestedImplicitInstantiations(
			pugi::xml_node node,
			const std::unordered_map<Family, std::string>& currentImplicit)
		{
			bool changed = false;
			for (auto child = node.first_child(); child; child = child.next_sibling()) {
				if (child.type() != pugi::node_element)
					continue;
				const std::string localName = child.name();
				const Family family = familyForNode(localName);
				if (family != Family::None && !isDefaultNodeName(localName)
					&& TrimAsciiWhitespace(child.attribute("template").as_string()).empty()) {
					auto it = currentImplicit.find(family);
					if (it != currentImplicit.end() && !it->second.empty()) {
						auto templateAttr = child.attribute("template");
						if (templateAttr)
							templateAttr.set_value(it->second.c_str());
						else
							child.append_attribute("template") = it->second.c_str();
						changed = true;
					}
				}
				changed = pinNestedImplicitInstantiations(child, currentImplicit) || changed;
			}
			return changed;
		}
		...
		bool convertAnonymousDefaultsToStatelessTemplates(pugi::xml_node systemNode)
		{
			...
			for (auto node = systemNode.first_child(); node; node = node.next_sibling()) {
				...
				if (family == Family::None) {
					// Pin instantiations nested in containers like <constraint-group>
					// to the implicit template active at this document position.
					changed = pinNestedImplicitInstantiations(node, currentImplicitTemplateByFamily) || changed;
					continue;
				}
		...
		// Recursively redirect every template="X" reference under `node` (including
		// constraints nested inside a <constraint-group>) to its canonical alias.
		bool redirectTemplateRefs(
			pugi::xml_node node,
			const std::unordered_map<std::string, std::string>& aliases)
		{
			...
			for (auto child = node.first_child(); child; child = child.next_sibling()) {
				if (child.type() != pugi::node_element)
					continue;
				const std::string localName = child.name();
				if (familyForNode(localName) != Family::None && !isDefaultNodeName(localName)) {
					auto templateAttr = child.attribute("template");
					auto aliasIt = aliases.find(TrimAsciiWhitespace(templateAttr.as_string()));
					if (aliasIt != aliases.end()) {
						if (templateAttr)
							templateAttr.set_value(aliasIt->second.c_str());
						else
							child.append_attribute("template") = aliasIt->second.c_str();
						changed = true;
					}
				}
				changed = redirectTemplateRefs(child, aliases) || changed;
			}
			return changed;
		}
		...
		bool deduplicateEquivalentTemplates(pugi::xml_document& doc, pugi::xml_node systemNode)
		{
			...
			// Redirect extends= on the (always top-level) default nodes.
			for (auto node = systemNode.first_child(); node; node = node.next_sibling()) {
				if (node.type() != pugi::node_element || !isDefaultNodeName(node.name()))
					continue;
				auto extendsAttr = node.attribute("extends");
				auto aliasIt = equivalentAliases.find(TrimAsciiWhitespace(extendsAttr.as_string()));
			...
			// Redirect template= on every instantiation, however deeply nested.
			changed = redirectTemplateRefs(systemNode, equivalentAliases) || changed;
		...
		// made a change. The template-pinning passes (pin anonymous defaults to
		// explicit names, shorten inheritance chains, deduplicate equivalent
		// templates) always run so the output never depends on concrete node order.
		bool applyImprovementPasses(pugi::xml_document& doc, pugi::xml_node sysNode)
		{
		...
		// Returns true when the source XML has schema errors — XSD structural
		// violations or Schematron error-role rules — i.e. what 'smp report'
		// counts as an error. Lets errors-only mode skip warning-only/clean files.
		bool sourceHasErrors(const std::string& srcXMLPath)
		{
			const XSDValidationResult xsd = ValidatePhysicsXMLWithXSD(srcXMLPath);
			if (!xsd.schemaSkipped && !xsd.violations.empty())
				return true;
			return ValidatePhysicsXMLWithSchematron(srcXMLPath).hasErrors;
		}
	...
	//
	// When errorsOnly is set, files whose source has no schema errors (only
	// warnings, or clean) are skipped before any cleanup.
	//
	...
	// (5) remove unused default nodes → (6) pin anonymous defaults to explicit
	// template names → (7) shorten inheritance chains → (8) deduplicate templates
	// with identical effective field sets → (9) remove empty defaults →
	// (10) merge adjacent anonymous defaults.
	bool GenerateImprovedXML(
		...
		bool errorsOnly)
	{
		if (errorsOnly && !sourceHasErrors(srcXMLPath))
			return false;
		...
		if (!applyImprovementPasses(doc, sysNode))
			return false;
