...
#define BTNUB_OPTIMIZATIONS
	...
	*/
	...
	static void btSolveL1_1(const btScalar *L, btScalar *B, int n, int lskip1)
	{
		...
		for (i = 0; i < n; i += 2) {
			...
			{
				__m128 ex_q1 = _mm_loadu_ps(ex);
				
				Z11 += _mm_cvtss_f32(_mm_dp_ps(_mm_loadu_ps(ell), ex_q1, 0xf1));
				Z21 += _mm_cvtss_f32(_mm_dp_ps(_mm_loadu_ps(ell + lskip1), ex_q1, 0xf1));
			}
	...
	*/
	...
	static void btSolveL1_2(const btScalar *L, btScalar *B, int n, int lskip1)
	{
		...
		for (i = 0; i < n; i += 2)
		{
			...
			{
				/* compute outer product and add it to the Z matrix */
				__m128 ell_p1 = _mm_loadu_ps(ell);
				__m128 ex_q1 = _mm_loadu_ps(ex);
				__m128 ell_lskip1_p2 = _mm_loadu_ps(ell + lskip1);
				__m128 ex_lskip1_q2 = _mm_loadu_ps(ex + lskip1);
				
				Z11 += _mm_cvtss_f32(_mm_dp_ps(ell_p1, ex_q1, 0xf1));
				Z12 += _mm_cvtss_f32(_mm_dp_ps(ell_p1, ex_lskip1_q2, 0xf1));
				Z21 += _mm_cvtss_f32(_mm_dp_ps(ell_lskip1_p2, ex_q1, 0xf1));
				Z22 += _mm_cvtss_f32(_mm_dp_ps(ell_lskip1_p2, ex_lskip1_q2, 0xf1));
				/* end of inner loop */
			}
			/* compute left-over iterations */
			j += 4;
	...
	void btFactorLDLT(btScalar *A, btScalar *d, int n, int nskip1)
	{
		...
		for (i = 0; i <= n - 2; i += 2)
		{
			...
			{
				__m128 ell_p1 = _mm_loadu_ps(ell);
				__m128 ell_nskip1_p2 = _mm_loadu_ps(ell+nskip1);
				__m128 dee_dd = _mm_loadu_ps(dee);
				__m128 p1_mul_dd_q1 = ell_p1 * dee_dd;
				__m128 p2_mul_dd_q2 = ell_nskip1_p2 * dee_dd;
				_mm_storeu_ps(ell, p1_mul_dd_q1);
				_mm_storeu_ps(ell+nskip1, p2_mul_dd_q2);
				Z11 += _mm_cvtss_f32(_mm_dp_ps(ell_p1, p1_mul_dd_q1, 0xf1));
				Z21 += _mm_cvtss_f32(_mm_dp_ps(ell_nskip1_p2, p1_mul_dd_q1, 0xf1));
				Z22 += _mm_cvtss_f32(_mm_dp_ps(ell_nskip1_p2, p2_mul_dd_q2, 0xf1));
			}
			/* compute left-over iterations */
			j += 4;
			for (; j > 0; j--) 
	...
	*/
	...
	void btSolveL1(const btScalar *L, btScalar *B, int n, int lskip1)
	{
		...
		for (i = 0; i <= n - 4; i += 4)
		{
			...
			{
				__m128 ell_p1 = _mm_loadu_ps(ell);
				__m128 ell_add_lskip1_p2 = _mm_loadu_ps(ell + lskip1);
				__m128 ell_add_lskip2_p3 = _mm_loadu_ps(ell + lskip2);
				__m128 ell_add_lskip3_p4 = _mm_loadu_ps(ell + lskip3);
				__m128 ex_q1 = _mm_loadu_ps(ex);

				ell_p1 = _mm_dp_ps(ell_p1, ex_q1, 0xf1);
				ell_add_lskip1_p2 = _mm_dp_ps(ell_add_lskip1_p2, ex_q1, 0xf2);
				ell_add_lskip2_p3 = _mm_dp_ps(ell_add_lskip2_p3, ex_q1, 0xf4);
				ell_add_lskip3_p4 = _mm_dp_ps(ell_add_lskip3_p4, ex_q1, 0xf8);

				ell_p1 = _mm_or_ps(ell_p1, ell_add_lskip2_p3);
				ell_add_lskip1_p2 = _mm_or_ps(ell_add_lskip1_p2, ell_add_lskip3_p4);
				ell_p1 = _mm_or_ps(ell_p1, ell_add_lskip1_p2);
				Z = _mm_add_ps(Z, ell_p1);
			}
			for (j += 4; j > 0; j--)
			{
				/* load p and q values */
		...
		for (; i < n; i++)
		{
			...
			for (j = i - 12; j >= 0; j -= 12)
			{
				__m128 ell_p1 = _mm_loadu_ps(ell);
				__m128 ex_q1 = _mm_loadu_ps(ex);
				__m128 ell_add_4_p2 = _mm_loadu_ps(ell+4);
				__m128 ex_add_4_q2 = _mm_loadu_ps(ex+4);
				__m128 ell_add_8_p3 = _mm_loadu_ps(ell+8);
				__m128 ex_add_8_q3 = _mm_loadu_ps(ex+8);

				Z11 += _mm_cvtss_f32(_mm_dp_ps(ell_p1, ex_q1, 0xf1));
				Z11 += _mm_cvtss_f32(_mm_dp_ps(ell_add_4_p2, ex_add_4_q2, 0xf1));
				Z11 += _mm_cvtss_f32(_mm_dp_ps(ell_add_8_p3, ex_add_8_q3, 0xf1));
	...
	*/
	...
	static void btSwapRowsAndCols(BTATYPE A, int n, int i1, int i2, [[maybe_unused]] int nskip, int do_fast_row_swaps)
	{
...
# endif
	...
	struct btLCP
	{
		...
		void transfer_i_to_N([[maybe_unused]] int i) { m_nN++; }			// because we can assume C and N span 1:i-1
...
# endif
		...
		{
			const int n = m_n;
			for (int k = 0; k<n; ++k) m_p[k] = k;		// initially unpermuted
		}
...
#define BTGETA(i,j) ((i > j) ? _BTGETA(i,j) : _BTGETA(j,i))
	...
	void btLDLTRemove(btScalar **A, const int *p, btScalar *L, btScalar *d,
		[[maybe_unused]] int n1, int n2, int r, int nskip, btAlignedObjectArray<btScalar>& scratch)
	{
...
#   endif
			...
			if (!only_transfer) 
			{
				...
				if (dir > 0) 
				{
					int *C = m_C;
					const int nC = m_nC;
					for (int j = 0; j<nC; ++j) a[C[j]] = -m_tmp[j];
				}
				else 
				{
					int *C = m_C;
	...
	bool DantzigSolver::solveMLCP(const btMatrixXu & A, const btVectorXu & b, btVectorXu& x, const btVectorXu & lo, const btVectorXu & hi, const btAlignedObjectArray<int>& limitDependency, [[maybe_unused]] int numIterations, [[maybe_unused]] bool useSparsity)
	{
		bool result = true;
		int n = b.rows();
		if (n)
