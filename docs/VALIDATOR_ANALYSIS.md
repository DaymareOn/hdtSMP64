...
This branch ships **only the read/report path**. The matching *repair* and *trim* commands
...
| **Runtime-accurate XML warnings** | out-of-`[0,1]` "factor" values reported as the clamped effective value; redundant tags suppressed unless redundant against the *effective inherited template* | [hdtTemplateDefaults.cpp](src/Validator/Utils/hdtTemplateDefaults.cpp) + `appendXmlViolationsToReport` |
